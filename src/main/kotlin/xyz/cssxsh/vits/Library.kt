...
public object Library {
    ...
    internal val cacheRoot: String by lazy {
        "${System.getProperty("user.home")}/.vits/"
    }
    @PublishedApi
    ...
    @PublishedApi
    internal const val VITS_LIBRARY_PATH_PROPERTY: String = "vits.library.path"

    private val libraryPath = System.getProperty(VITS_LIBRARY_PATH_PROPERTY)
    ...
    private fun loadLibraryOrCopy(library: File) {
        try {
            ...
            if (e.message?.contains("already loaded in another classloader") == true) {
                copyDir = Files.createTempDirectory("vits").toFile()
    ...
    private fun unpackIfNeeded(dest: File, resourceName: String): File {
        ...
        if (!file.exists()) {
            val tempFile = File.createTempFile("vits", "", dest)
